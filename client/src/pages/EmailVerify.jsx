...
const EmailVerify = () => {
  ...
  return (
    <div className="flex items-center justify-center min-h-screen  bg-[#00003C]">
      ...
      <form onSubmit={onSubmitHandler} className='bg-slate-700 p-8 rounded-lg shadow-lg w-96 text-sm'>
        <h1 className='text-white text-2xl font-semibold text-center mb-4'>Email Verify OTP</h1>
        <p className='text-center mb-6 text-indigo-300'>Enter the 6-digit code sent to your email.</p>

        <div className='flex justify-between mb-8 ' onPaste={handlPaste}>
          {Array(6).fill(0).map((_, i) => (
            <input type='text' maxLength='1' key={i} required className='w-12 h-12 text-center bg-slate-900 text-white rounded-lg' 
        ...
        <button className='w-full py-3 bg-indigo-500 hover:bg-indigo-600 text-white rounded-lg hover:cursor-pointer'>Verify Email</button>
      </form>
